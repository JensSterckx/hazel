...
module.exports = ({ cache, config }) => {
  ...
  exports.updateFile = async (req, res) => {
    const { platform: platformName, version, filename } = req.params
    ...
    // Get the latest version from the cache
    const latest = await loadCache()
    const result = latest.files.nupkgs.find(obj => obj.name === filename)

    if (result !== undefined) {
      console.log('Redirecting file to GitHub CDN: ' + result.realurl)
      res.setHeader('Location', result.realurl)
      send(res, 302)
    }
    else {
      send(res, 404, 'Bad file')
    }
  }
