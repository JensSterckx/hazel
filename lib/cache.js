...
module.exports = class Cache {
  ...
  async cacheReleaseList(url) {
    ...
    //We will get error 400 due to the 304 redirect to amazon, wich will tell us to not use double authentication
    //So redirect is set to manual, so we can get the location, and resend without the authorization header
    ...
    const { status, body } = await retry(
      async () => {
        const responseRedir = await fetch(url, options)
        //We will get a 302 redirect and location header in response
        const realUrl = responseRedir.headers.get('Location')
        const response = await fetch(realUrl, { Accept: 'application/octet-stream' })
        if (response.status !== 200) {
          throw new Error(
            `Tried to cache RELEASES, but failed fetching ${url}, status ${response.status}, token ${token}`
          )
        }
    ...
    //This replaces the RELEASES part in the url with the file of amazon CDN
    //We do this in refreshCache()

    /*
    for (let i = 0; i < matches.length; i += 1) {
      const nuPKG = url.replace('RELEASES', matches[i])
      console.log("nuPKG is " + nuPKG)
  ...
  async refreshCache() {
    ...
    //Get a direct link to be able to redirect when squirrel asks for file
    for (const asset of release.assets) {
      ...
      //Get redirect url of the actual file
      const redirect = 'manual'
      const headers = { Accept: 'application/octet-stream' }
      const options = { headers, redirect }
